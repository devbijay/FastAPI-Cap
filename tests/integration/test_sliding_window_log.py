...
@pytest.mark.asyncio
async def test_sliding_log_allows_within_limit(app):
...
@pytest.mark.asyncio
async def test_sliding_log_blocks_over_limit(app):
    async with httpx.AsyncClient(
        transport=ASGITransport(app=app), base_url="http://test"
    ) as client:
...
@pytest.mark.asyncio
async def test_sliding_log_resets_after_window(app):
    async with httpx.AsyncClient(
        transport=ASGITransport(app=app), base_url="http://test"
    ) as client:
        await client.get("/ping")
...
@pytest.mark.asyncio
async def test_sliding_log_separate_keys(app):
    async with httpx.AsyncClient(
        transport=ASGITransport(app=app), base_url="http://test"
    ) as client:
        assert (await client.get("/ping")).status_code == 200
...
@pytest.mark.asyncio
async def test_sliding_log_multiple_limiters():
    app = FastAPI()
