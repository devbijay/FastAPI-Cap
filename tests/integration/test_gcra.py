...
def app():
    app = FastAPI()
    limiter = GCRARateLimiter(burst=2, tokens_per_second=1)
...
@pytest.mark.asyncio
async def test_gcra_allows_within_burst(app):
...
@pytest.mark.asyncio
async def test_gcra_blocks_when_over_burst(app):
    async with httpx.AsyncClient(
        transport=ASGITransport(app=app), base_url="http://test"
    ) as client:
...
@pytest.mark.asyncio
async def test_gcra_allows_after_wait(app):
    async with httpx.AsyncClient(
        transport=ASGITransport(app=app), base_url="http://test"
    ) as client:
        await client.get("/ping")
        ...
        r3 = await client.get("/ping")
        assert r3.status_code == 429
        await asyncio.sleep(2.1)  # Wait for enough tokens to accumulate
        r4 = await client.get("/ping")
        assert r4.status_code == 200


@pytest.mark.asyncio
async def test_gcra_separate_keys(app):
    async with httpx.AsyncClient(
        transport=ASGITransport(app=app), base_url="http://test"
    ) as client:
        assert (await client.get("/ping")).status_code == 200
...
@pytest.mark.asyncio
async def test_gcra_multiple_limiters():
    app = FastAPI()
    limiter1 = GCRARateLimiter(burst=1, tokens_per_second=1, prefix="short")
    limiter2 = GCRARateLimiter(burst=2, tokens_per_second=2, prefix="long")
