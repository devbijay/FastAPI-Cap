...
async def test_allows_within_limit(redis_ready):
    limiter = SlidingWindowLogRateLimiter(limit=2, window_seconds=3)
...
async def test_separate_keys(redis_ready):
    limiter = SlidingWindowLogRateLimiter(limit=1, window_seconds=3)
...
async def test_custom_key_func(redis_ready):
    ...
    limiter = SlidingWindowLogRateLimiter(
        limit=1, window_seconds=3, key_func=custom_key_func
    )
    request = DummyRequest()
    response = DummyResponse()
    await limiter(request, response)  # Allowed
...
async def test_custom_on_limit(redis_ready):
    ...
    limiter = SlidingWindowLogRateLimiter(
        limit=1, window_seconds=3, on_limit=custom_on_limit
    )
    request = DummyRequest()
    response = DummyResponse()
    await limiter(request, response)  # Allowed
...
async def test_prefix_isolation(redis_ready):
    limiter1 = SlidingWindowLogRateLimiter(limit=1, window_seconds=3, prefix="a")
    limiter2 = SlidingWindowLogRateLimiter(limit=1, window_seconds=3, prefix="b")
    request = DummyRequest()
    response = DummyResponse()
...
@pytest.mark.asyncio
async def test_multiple_sliding_log_limiters(redis_ready):
    limiter1 = SlidingWindowLogRateLimiter(limit=1, window_seconds=3, prefix="short")
    limiter2 = SlidingWindowLogRateLimiter(limit=2, window_seconds=5, prefix="long")
    request = DummyRequest()
    response = DummyResponse()
